...
void CGameFramework::CreateDirect3DDevice()
{
	...
	m_hFenceEvent = ::CreateEvent(NULL, FALSE, FALSE, NULL);  // 이벤트가 실행되면 이벤트의 값을 FALSE가 되도록 설정한다.
...
void CGameFramework::CreateSwapChain()
{
	...
	//m_pdxgiFactory->CreateSwapChainForHwnd(m_pd3dCommandQueue, m_hWnd, &dxgiSwapChainDesc, &dxgiSwapChainFullScreenDesc, NULL, (IDXGISwapChain1**)&m_pdxgiSwapChain);  // 스왑 체인 생성

	//m_pdxgiFactory->MakeWindowAssociation(m_hWnd, DXGI_MWA_NO_ALT_ENTER);  // Alt + Enter의 동작을 비활성화

	//m_nSwapChainBufferIndex = m_pdxgiSwapChain->GetCurrentBackBufferIndex();  // 스왑 체인의 현재 후면 버퍼 인덱스를 저장
	...
	dxgiSwapChainDesc.BufferCount = m_nSwapChainBuffers;
	dxgiSwapChainDesc.SwapEffect = DXGI_SWAP_EFFECT_FLIP_DISCARD;
	dxgiSwapChainDesc.Flags = DXGI_SWAP_CHAIN_FLAG_ALLOW_MODE_SWITCH;  // 화면 모드 전환
...
void CGameFramework::ProcessInput()
{
	...
	if (::GetCapture() == m_hWnd) {
		::SetCursor(NULL);  // 마우스 커서를 보이지 않게 한다.
